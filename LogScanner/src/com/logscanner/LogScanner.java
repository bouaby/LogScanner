...
public class LogScanner {
	...
	private LogScanner() throws Exception {
		...
		//Archive any existing Report File
		if (!Utils.isEmpty(props.getArchiveReportFile())
				&& "true".equalsIgnoreCase(props.getArchiveReportFile()))
		Utils.archiveReportFile(props, sbMainReportContent);
		...
		dStartDate = new Date();
		String emailStatus = "";
		if (!Utils.isEmpty(props.getEmailReport())
				&& "true".equalsIgnoreCase(props.getEmailReport())) {
			dStartDate = new Date();
			sStatusMsg = "Sending email to \""+props.getEmailAddress()+"\" started";
			...
			try {
				...
				email.setAttachmentsProperties(
						props.getMaxEmailBodySizeBytes(),
						props.getMaxEmailAttachedFileSizeBytes());
				if (!Utils.isEmpty(props.getEmailHTML())
						&& "true".equalsIgnoreCase(props.getEmailHTML()))
					sMime = MIME_HTML;
				email.send(sMime, props.getEmailAddress(), null, null,
						props.getEmailAddress(), props.getEmailSubject(),
		...
		// Clean up
		alTranslateThreadsHandler.clear();
		alTranslateThreadsHandler = null;
		alScanThreadsHandler.clear();
