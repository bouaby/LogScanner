...
public class Utils {
	...
	protected final static String sSTD_DATE_FORMAT = "MM-dd-yyyy hh:mm:ss a";
	...
	protected static String getDateFromLine(String aLine,
		...
		try {
			...
			// Check if date is null because dates are expressed as 1-9 instead
			// of 01-09
			// This applies 5 times to: month, date, hour, minute, second
			if (lTempDate == null) {
				for (int i = 1; i < 6; i++) {
					lLogFileDateEnd--;
					lTempDateValue = lFileLine.substring(lLogFileDateStart,
							lLogFileDateEnd);

					lTempDate = convertToDate(lTempDateValue,
							lLogFileDateFormat);
					if (lTempDate != null) {
						break;
					}
